...
def flag_to_int_array(flag: bytes) -> typing.Tuple[int, int]:
    return int.from_bytes(flag[0:1], byteorder='big'), int.from_bytes(flag[1:], byteorder='big')
...
class SceneData:
    ...
    def __init__(self, file_path: str, timeline_regex: str = None):
    ...
    def content_str(self) -> str:
        try:
            ...
            raise SceneData.MemoryError(f"File is to big, content.decode('utf-8', errors='ignore') failed")
    ...
    def duration(self, value: float):
        ...
        if not self.has_timeline():
            ...
            raise SceneData.ValueError("Scene data is Static and does not have a duration")
        elif value <= 0:
    ...
    def get_timeline_info(self) -> typing.Tuple[str, str, float]:
        return self.timeline_status, self.image_type, self.duration
    
    def get_timeline_xml(self, raise_exception: bool=False) -> typing.Optional[bytes]:
        """
        Raises
        ------
    ...
    def get_timeline_xml_tree(self, raise_exception: bool=False) -> typing.Optional[et.ElementTree]:
    ...
    def _get_timeline_byte_vars(self, timeline) -> typing.Tuple[bytes, bytes]:
        """Return the length flag and the timeline byte length"""
        num_bytes = (len(timeline).bit_length() + 7) // 8
        length_flag = (self._timeline_length_flag + (num_bytes if num_bytes <= 3 else 3)).to_bytes(1, byteorder='big')
    ...
    def _is_scene_data(self) -> bool:
        ...
        return os.path.splitext(self.file_path)[1] == ".png" and b"KStudio" in self._content
        
    def _check_timeline(self) -> typing.Tuple[str, str, float]:
            # Author: https://github.com/GameProgram777/KKTimelineDetector
            """
