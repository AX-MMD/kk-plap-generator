...
If two components have the same name in any of the pages, the generator will try to combine them as one.

: MAC (MultiActivableComponent) :

-- Name: Components with the same name will be combined if possible.
...
-- Name: Components with the same name will be combined if possible (will check every pages).
-- Min Value: The minimum bulge size for the Preg+ plugin.
-- Max Value: The maximum bulge size for the Preg+ plugin.
